...
# This image includes Python 3.10, CUDA 12.1, and cuDNN 8, matching PyTorch 2.3.1 requirements.
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    libgl1-mesa-glx \
    libglib2.0-0 \
    # Add any other essential system dependencies here (e.g., ca-certificates for HTTPS)
    ca-certificates \
    && apt-get clean \
    && rm -rf /var/lib/apt/lists/*

# Step 4: Upgrade pip (using the python from the base image)
# The base PyTorch image should have a compatible Python.
RUN python -m pip install --upgrade pip

# Step 5: Create a working directory
WORKDIR /opt/app
# Create a non-root user and group (optional but good security practice)
# RUN groupadd -r jovyan && useradd -r -g jovyan -m -s /bin/bash jovyan
# USER jovyan
# WORKDIR /home/jovyan/work
# If using a non-root user, ensure pip installs to a user-writable location or adjust permissions.
# For simplicity in Kubeflow where user context is often managed, running as root is common.

# Step 6: Copy requirements file and install Python packages
COPY requirements.txt .
# The PyTorch version in requirements.txt should match the CUDA in the base image.
...
# If you used a non-root user 'jovyan' and WORKDIR /home/jovyan/work:
# CMD ["jupyter", "lab", "--ip=0.0.0.0", "--port=8888", "--no-browser", "--NotebookApp.token=''", "--NotebookApp.password=''", "--NotebookApp.notebook_dir=/home/jovyan/work"]
